...
cells = [
    ...
    md("### 📋 使うデータ：`sales_btob.csv`（架空のBtoB商談400件）\n"
...
cells = [
    ...
    md("### 📋 使うデータ：`monthly_kpi.csv`（36か月の月次KPI）\n"
       "1行＝1か月。右肩上がりの**トレンド**と、年末が高い**季節性**を含みます。\n\n"
       "| 月 | 売上 | 新規リード | 受注数 |\n"
       "|---|---|---|---|\n"
       "| 2023-01 | 760 | 68 | 15 |\n"
       "| 2023-02 | 766 | 82 | 13 |\n"
       "| 2023-03 | 860 | 80 | 19 |\n\n"
       "（売上は万円。この `売上` を予測対象にします）"),
    md("## 1. まず眺める（トレンド＋季節性）"),
...
cells = [
    ...
    md("### 📋 使うデータ：`btob_customers.csv`（顧客320件）\n"
       "1行＝顧客1人。`最終購入日`から Recency（最近購入したか）を計算します。\n\n"
       "| 顧客ID | 業界 | 購入回数 | 累計売上 | 最終購入日 | （正解セグメント） |\n"
       "|---|---|---|---|---|---|\n"
       "| C0124 | 医療 | 4 | 3,843,000 | 2025-11-25 | 一般 |\n"
       "| C0111 | 金融 | 5 | 2,323,000 | 2025-07-05 | 一般 |\n"
       "| C0081 | 製造 | 3 | 3,007,000 | 2025-07-11 | 一般 |\n\n"
       "> ℹ️ `（正解セグメント）` はデータ作成時に付けた**答え合わせ用ラベル**。"
       "クラスタリングでは使わず、結果の検証にだけ使えます。"),
    md("## 1. RFM指標を作る\n\n"
