...
| `05_実践_BtoBマーケ` | **01-03** 売上分析・マーケファネル・A/Bテスト／**04** 顧客スコアリング(標準化)・**05** 広告クリックの予測(単回帰)・**06** 受注率の評価(区間推定・検定) | 4〜3級の応用（04は3級・05は3級・06は2級） |
| `06_発展_マーケ分析` | **01-03** ドライバー分析(回帰/ロジスティック)・時系列予測(指数平滑)・顧客セグメント(RFM×K-means)／**04** チャネル別クリック率の比較(t検定)・**05** チャネル別獲得率の比較(分散分析)・**06** 売上ドライバーの分析(重回帰) | 発展（01-03は準1級〜DS／04-06は2級の検定・回帰） |
| `07_ホビー_陶芸3D` | **00** 陶芸×3Dの使いどころとツールの選び方／FreeCAD・Blender・SolidPythonで器づくり／**04** 実用ツール(スタンプ・こて・抜き型) | お楽しみ |
