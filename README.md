...
| `05_実践_BtoBマーケ` | **01-03** 売上分析・マーケファネル・A/Bテスト／**04** 顧客スコアリング(標準化)・**05** 単回帰・**06** 受注率の区間推定と検定 | 4〜3級の応用（04-06は読む3級・本質3〜2級） |
| `06_発展_マーケ分析` | **01-03** ドライバー分析(回帰/ロジスティック)・時系列予測(指数平滑)・顧客セグメント(RFM×K-means)／**04** 因果推論(傾向スコア/IPW)・**05** 生存時間解析(KM/Cox)・**06** ベイズ統計(事後分布/信用区間) | 発展（2級の先・準1級〜DS。04-06は読む準1級・本質1級） |
